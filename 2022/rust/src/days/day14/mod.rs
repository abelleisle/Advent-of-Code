...
pub fn sand_fall(cave: &mut Input, fall: bool, animate: bool, fps: u64) -> bool {
    let mut sand = (500, 0);
    let floor = cave.iter().map(|k| k.0.1).max().unwrap();
...
pub fn part1(mut input: Input) -> usize {
    loop {
        if !sand_fall(&mut input, false, true, 60) {
            break;
        }
    }
...
pub fn part2(mut input: Input) -> usize {
    loop {
        if !sand_fall(&mut input, true, false, 0) {
            break;
        }
    }
